...
/**
 * GENERATE GLOBAL WORK DONE TABLE PDF
 */
export const generateGlobalTableDataPDF = (data: any, stream: any) => {
  const { clinic, records } = data;
  const doc = new PDFDocument({
    margin: 0,
    size: "A4",
    bufferPages: true
  ...
  doc
    ...
    .opacity(0.8)
    .fontSize(10)
    .text("ALL PATIENTS", rightAlignX, 48, { align: "right", width: 250 })
    .fontSize(8.5)
    .font("Helvetica")
    .text(`Generated On: ${moment().format('DD/MM/YYYY')}`, rightAlignX, 96, { align: "right", width: 250 });
  ...
  doc
    ...
    .font("Helvetica-Bold")
    .text("DATE", colX.date, tableTop + 9)
    .text("PATIENT", colX.patient, tableTop + 9)
    .text("TOOTH", colX.tooth, tableTop + 9)
    .text("TREATMENT", colX.treatment, tableTop + 9)
    .text("DOCTOR", colX.doctor, tableTop + 9)
    .text("FEES", colX.fees, tableTop + 9, { width: 50, align: "right" })
    .text("PAID", colX.paid, tableTop + 9, { width: 50, align: "right" })
    .text("STATUS", colX.status, tableTop + 9, { width: 45, align: "center" });
...
export const generateGlobalAccountabilityPDF = (data: any, stream: any, selectedColumns?: string[], fromDate?: string, toDate?: string) => {
  ...
  const allColumns = [
    { key: "date", label: "Bill Date", width: 45 },
    { key: "receiptNumber", label: "RECEIPT", width: 40 },
    { key: "patient", label: "PATIENT", width: 60 },
    { key: "tooth", label: "TOOTH", width: 30 },
    { key: "treatmentCode", label: "CODE", width: 60 },
    { key: "treatment", label: "TREATMENT", width: 60 },
    { key: "doctor", label: "DOCTOR", width: 55 },
    { key: "fees", label: "FEES", width: 35 },
    { key: "discount", label: "DISC", width: 30 },
  ...
  sortedRecords.forEach((row: any) => {
    ...
    if (colX.fees) {
      const feesAmount = row.amount || 0;
      doc.fillColor(COLORS.textMain).font("Helvetica-Bold").text(feesAmount.toString(), colX.fees, y + 8);
    }
    if (colX.discount) {
      doc.fillColor(COLORS.danger).font("Helvetica-Bold").text((row.discount || 0).toString(), colX.discount, y + 8);
    }
  ...
  doc
    ...
    .fontSize(7)
    .font("Helvetica")
    .text("Generated by Clinic System", MARGIN, footerY + 10, { align: "left", lineBreak: false })
    .text(`Page Count: ${doc.bufferedPageRange().count}`, PAGE_WIDTH - MARGIN - 100, footerY + 10, { align: "right", width: 100, lineBreak: false });
...
export const generateLegacyWorkCompPDF = (data: any[], stream: any, fromDate?: string, toDate?: string) => {
  const PDFDocument = require("pdfkit");
  ...
  doc
    ...
    .font("Helvetica-Bold")
    .fontSize(20)
    .text("Historical Work Done", MARGIN, 25);
  ...
  doc
    ...
    .opacity(1)
    .fontSize(10)
    .text(`Records: ${data.length} (Max 2000)`, rightAlignX, 45, { align: "right", width: 250 })
    .text(`Generated On: ${moment().format('DD/MM/YYYY')}`, rightAlignX, 57, { align: "right", width: 250 });

  const allColumns = [
    { key: "date", label: "DATE", width: 55 },
    { key: "patCode", label: "PAT CODE", width: 65 },
    { key: "patName", label: "PATIENT NAME", width: 100 },
    { key: "doctor", label: "DOCTOR", width: 80 },
    { key: "feeDue", label: "FEE DUE", width: 50 },
    { key: "feeDis", label: "FEE DIS", width: 50 },
  ...
  const totalDefaultWidth = allColumns.reduce((sum, c) => sum + c.width, 0);
  const extraSpace = Math.max(0, CONTENT_WIDTH - 10 - totalDefaultWidth);
  const expandBy = allColumns.length > 0 ? extraSpace / allColumns.length : 0;
  ...
  data.forEach((row: any) => {
    const patientName = row.patientId?.name || "Unknown";
    const docName = row.doctorId?.name || row.legacyDocCode || "Unknown";

    doc.font("Helvetica").fontSize(8);
    let maxH = 12;
    doc.font("Helvetica-Bold");
    maxH = Math.max(maxH, doc.heightOfString(patientName, { width: colW.patName - 5 }));
    doc.font("Helvetica");
    ...
    doc.text(new Date(row.wrk_date).toLocaleDateString('en-IN'), colX.date, y + 8);
    doc.text(row.legacyPatCode, colX.patCode, y + 8, { width: colW.patCode - 5, ellipsis: true });
    doc.font("Helvetica-Bold").text(patientName, colX.patName, y + 8, { width: colW.patName - 5 }).font("Helvetica");
    doc.text(docName, colX.doctor, y + 8, { width: colW.doctor - 5, ellipsis: true });

    doc.font("Helvetica-Bold").text((row.fee_due || 0).toString(), colX.feeDue, y + 8).font("Helvetica");
    doc.text((row.fee_dis || 0).toString(), colX.feeDis, y + 8);

    doc.save();
    const stageY = y + (currentDynamicRowH / 2) - 6;
    doc.fillColor(COLORS.brand).roundedRect(colX.stage, stageY, 25, 12, 3).fill();
    doc.fillColor(COLORS.white).fontSize(6).font("Helvetica-Bold").text((row.treat_stage || "-"), colX.stage, stageY + 4, { width: 25, align: "center" });
    doc.restore();
...
const createLegacyPDFHeader = (doc: any, title: string, dataLength: number, PAGE_WIDTH: number, MARGIN: number, COLORS: any, moment: any) => {
  doc
    .fillColor(COLORS.brand)
    .font("Helvetica-Bold")
    .fontSize(20)
  ...
  doc
    ...
    .opacity(1)
    .fontSize(10)
    .text(`Records: ${dataLength}`, rightAlignX, 45, { align: "right", width: 250 })
    .text(`Generated On: ${moment().format('DD/MM/YYYY')}`, rightAlignX, 57, { align: "right", width: 250 });
};

const renderLegacyTable = (doc: any, data: any[], allColumns: any[], rowRenderer: Function, COLORS: any, PAGE_WIDTH: number, PAGE_HEIGHT: number, MARGIN: number, CONTENT_WIDTH: number) => {
  const colX: Record<string, number> = {};
  const colW: Record<string, number> = {};
  let currentX = MARGIN + 5;
  ...
  data.forEach((row: any) => {
    ...
    doc.fillColor(COLORS.textMain).font("Helvetica").fontSize(8);
    rowRenderer(doc, row, patientName, docName, colX, colW, y, currentDynamicRowH, COLORS);
...
export const generateLegacyToothWorkPDF = (data: any[], stream: any, fromDate?: string, toDate?: string) => {
  const PDFDocument = require("pdfkit");
  const moment = require("moment");
  const doc = new PDFDocument({ margin: 0, size: "A4", bufferPages: true });
  doc.pipe(stream);

  createLegacyPDFHeader(doc, "Historical Tooth Work", data.length, 595.28, 20, LEGACY_COLORS, moment);

  const allColumns = [
    { key: "date", label: "DATE", width: 60 },
    { key: "patCode", label: "PAT CODE", width: 70 },
    { key: "patName", label: "PATIENT NAME", width: 110 },
    { key: "doctor", label: "DOCTOR", width: 90 },
    { key: "tooth", label: "TOOTH", width: 50 },
    { key: "treatment", label: "TREATMENT", width: 100 },
  ];

  const rowRenderer = (doc: any, row: any, patientName: string, docName: string, colX: any, colW: any, y: number, h: number, COLORS: any) => {
    doc.text(row.wrkdate ? new Date(row.wrkdate).toLocaleDateString('en-IN') : "-", colX.date, y + 8);
    doc.text(row.legacyPatCode, colX.patCode, y + 8, { width: colW.patCode - 5, ellipsis: true });
    doc.font("Helvetica-Bold").text(patientName, colX.patName, y + 8, { width: colW.patName - 5 }).font("Helvetica");
    doc.text(docName, colX.doctor, y + 8, { width: colW.doctor - 5, ellipsis: true });
    doc.text((row.ToothNoS || "-"), colX.tooth, y + 8);
    doc.text((row.name || row.descript || "-"), colX.treatment, y + 8, { width: colW.treatment - 5, ellipsis: true });
  };
...
export const generateLegacyTransactionPDF = (data: any[], stream: any, fromDate?: string, toDate?: string) => {
  const PDFDocument = require("pdfkit");
  const moment = require("moment");
  const doc = new PDFDocument({ margin: 0, size: "A4", bufferPages: true });
  doc.pipe(stream);

  createLegacyPDFHeader(doc, "Historical Transactions", data.length, 595.28, 20, LEGACY_COLORS, moment);

  const allColumns = [
    { key: "date", label: "DATE", width: 60 },
    { key: "workDate", label: "WORK DATE", width: 60 },
    { key: "patCode", label: "PAT CODE", width: 70 },
    { key: "patName", label: "PATIENT NAME", width: 110 },
    { key: "doctor", label: "DOCTOR", width: 90 },
    { key: "feeRec", label: "FEE RECEIVED", width: 70 },
  ];

  const rowRenderer = (doc: any, row: any, patientName: string, docName: string, colX: any, colW: any, y: number, h: number, COLORS: any) => {
    doc.text(row.date ? new Date(row.date).toLocaleDateString('en-IN') : "-", colX.date, y + 8);
    doc.text(row.wrk_date ? new Date(row.wrk_date).toLocaleDateString('en-IN') : "-", colX.workDate, y + 8);
    doc.text(row.legacyPatCode, colX.patCode, y + 8, { width: colW.patCode - 5, ellipsis: true });
    doc.font("Helvetica-Bold").text(patientName, colX.patName, y + 8, { width: colW.patName - 5 }).font("Helvetica");
    doc.text(docName, colX.doctor, y + 8, { width: colW.doctor - 5, ellipsis: true });
    doc.font("Helvetica-Bold").fillColor(COLORS.success).text(`Rs. ${row.fee_rec || 0}`, colX.feeRec, y + 8).font("Helvetica").fillColor(COLORS.textMain);
  };
...
export const generateLegacyWorkFeePDF = (data: any[], stream: any, fromDate?: string, toDate?: string) => {
  const PDFDocument = require("pdfkit");
  const moment = require("moment");
  const doc = new PDFDocument({ margin: 0, size: "A4", bufferPages: true });
  doc.pipe(stream);

  createLegacyPDFHeader(doc, "Historical Fees", data.length, 595.28, 20, LEGACY_COLORS, moment);
  ...
  const allColumns = [
    ...
    { key: "patName", label: "PATIENT NAME", width: 110 },
    { key: "doctor", label: "DOCTOR", width: 90 },
    { key: "feeDue", label: "FEE DUE", width: 60 },
    { key: "feeDis", label: "FEE DIS", width: 60 },
    { key: "stage", label: "STAGE", width: 50 },
  ];
  ...
  const rowRenderer = (doc: any, row: any, patientName: string, docName: string, colX: any, colW: any, y: number, h: number, COLORS: any) => {
    ...
    doc.fillColor(COLORS.brand).roundedRect(colX.stage, stageY, 25, 12, 3).fill();
    doc.fillColor(COLORS.white).font("Helvetica-Bold").fontSize(7).text(row.treat_stage || "-", colX.stage, stageY + 3, { width: 25, align: "center" });
  };
...
/**
 * GENERATE GLOBAL TREATMENT TABLE PDF
 */
export const generateGlobalTreatmentTableDataPDF = (data: any, stream: any) => {
  const { clinic, records } = data;
  const doc = new PDFDocument({
    margin: 0,
    size: "A4",
  ...
  // --- STYLE TOKENS ---
  const COLORS = {
    brand: "#4c1d95", // purple theme for treatments
    brandLight: "#7c3aed",
    textMain: "#111827",
    textMuted: "#4b5563",
    bgLight: "#f8fafc",
  ...
  doc
    ...
    .font("Helvetica-Bold")
    .fontSize(13)
    .text("GLOBAL TREATMENT REPORT", rightAlignX, 25, { align: "right", width: 250 })
    .opacity(0.8)
    .fontSize(10)
    .text("ALL PATIENTS", rightAlignX, 48, { align: "right", width: 250 })
    .fontSize(8.5)
  ...
  records.forEach((record: any) => {
    ...
    const date = moment(record.treatmentDate || record.createdAt).format('DD/MM/YYYY');
    const patientName = (record.patient as any)?.name || "N/A";
    const treatment = record.treatmentPlan || record.notes || "Clinical Note";
    const toothStr = record.tooth || "N/A";
    const doctor = (record.doctor as any)?.name || "N/A";
    const clinicalStatus = (record.status || "pending").toLowerCase();
    ...
    doc
      ...
      .font("Helvetica-Bold")
      .text(date, colX.date, y + 9)
      .text(patientName, colX.patient, y + 9, { width: 110, height: 12, ellipsis: true })
      .font("Helvetica")
      .text(toothStr, colX.tooth, y + 9, { width: 40, height: 12, ellipsis: true })
    ...
    let statusColor = COLORS.success;
    if (clinicalStatus === "pending") statusColor = "#f59e0b";
    if (clinicalStatus === "incomplete") statusColor = COLORS.danger;
  ...
  doc
    ...
    .font("Helvetica")
    .text(`Generated by ${clinic?.company_name || "Clinic System"}`, MARGIN, footerY + 10, { align: "left", lineBreak: false })
    .text(`Total Records: ${records.length}`, PAGE_WIDTH - MARGIN - 100, footerY + 10, { align: "right", width: 100, lineBreak: false });
