...
import { uploadFile, uploadTutorialFile } from "../uploadDoc.repository";
...
export const uploadTutorialDocument = async (data: any) => {
  try {
    if (data.file?.filename && data.file?.buffer && data.file) {
      const url = await uploadTutorialFile(data.file);
      return {
        status: "success",
        data: url,
        statusCode: statusCode.success,
        message: "Tutorial document has been uploaded",
      };
    }
  } catch (err: any) {
    return {
